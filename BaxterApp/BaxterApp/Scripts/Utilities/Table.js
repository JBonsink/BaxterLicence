function simpleBootstrapTable(headersTop, headersLeft, rows) {
    var table = $('<table>').addClass('table table-striped');
    var thead = $('<thead>').appendTo(table);
    var headerRow = $('<tr>').appendTo(thead);
    var body = $('<tbody>').appendTo(table);

    $.each(headersTop, function (i, header) {
        headerRow.append($('<th>').text(header))
    });

    $.each(rows, function (i, row) {
        body.append(getRow(row, i));
    });

    function getRow(columns, i) {
        var row = $('<tr>');

        if (headersLeft[i])
            row.append($('<th>').text(headersLeft[i]));
